...
const getReverseGeoCoding = async (address, postcode, city, callback) => {
    ...
    try {
        const response = await axios.get(`https://api.mapbox.com/geocoding/v5/mapbox.places/${address}%20${postcode}%20${city}.json?types=address&access_token=${mapBoxToken}`)
        return response
    } catch(err) {
        callback(err)
    }
